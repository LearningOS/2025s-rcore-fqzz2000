...
impl TaskControlBlock {
    ...
    /// insert a area into memory set
    pub fn insert_memory_set(&self, start_va: VirtAddr, end_va: VirtAddr, permission: MapPermission) {
        let mut inner = self.inner.exclusive_access();
        inner.memory_set.insert_framed_area(start_va, end_va, permission);
    }

    /// unmap a area from memory set    
    pub fn unmap_memory_set(&self, start: usize, len: usize) {
        let mut inner = self.inner.exclusive_access();
        inner.memory_set.unmap(start, len);
    }
        /// set priority
